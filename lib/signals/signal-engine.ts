...
interface SignalInput {
  ...
  newsBoost?: number // points from news sentiment
}
...
export function detectSignal(input: SignalInput): TradeSignal | null {
  ...
  if (quote.changePercent > 3) {
    ...
    triggers.push({
      ...
      strength: 'strong',
      description: `Up ${quote.changePercent.toFixed(2)}% today - strong momentum`,
    ...
    triggers.push({
      type: 'MOMENTUM_UP',
      strength: 'moderate',
      description: `Up ${quote.changePercent.toFixed(2)}% today`,
      value: quote.changePercent
    })
    ...
    triggers.push({
      ...
      strength: 'strong',
      description: `Down ${Math.abs(quote.changePercent).toFixed(2)}% today - bearish momentum`,
      value: quote.changePercent
    })
  } else if (quote.changePercent < -1.5) {
    score += 8
    ...
    triggers.push({
      type: 'MOMENTUM_DOWN',
      strength: 'moderate',
      description: `Down ${Math.abs(quote.changePercent).toFixed(2)}% today`,
      value: quote.changePercent
    })
  }
  ...
  if (indicators.rsi < 30 && indicators.rsiTrend === 'rising') {
    ...
    triggers.push({
      ...
      strength: 'strong',
      description: `RSI at ${indicators.rsi.toFixed(1)} bouncing from oversold`,
    ...
    triggers.push({
      type: 'RSI_REVERSAL',
      strength: 'moderate',
      description: `RSI at ${indicators.rsi.toFixed(1)} turning up from low levels`,
      value: indicators.rsi
    })
    ...
    isBullish = false
    triggers.push({
      type: 'RSI_OVERBOUGHT_REJECTION',
      strength: 'strong',
      description: `RSI at ${indicators.rsi.toFixed(1)} rejecting from overbought`,
      value: indicators.rsi
    })
    ...
    triggers.push({
      type: 'RSI_FADING',
      strength: 'moderate',
      description: `RSI at ${indicators.rsi.toFixed(1)} fading from high levels`,
      value: indicators.rsi
    })
  }
  ...
  if (indicators.macdCrossover === 'bullish') {
    ...
    triggers.push({
      ...
      strength: 'strong',
      description: 'MACD crossed above signal line — momentum shifting bullish',
    ...
    triggers.push({
      ...
      strength: 'strong',
      description: 'MACD crossed below signal line — momentum shifting bearish',
      value: indicators.macd
    })
  }
  ...
  if (quote.price > indicators.ema9 && indicators.ema9 > indicators.ema21) {
    ...
    triggers.push({
      ...
      strength: 'strong',
      description: 'Price > 9 EMA > 21 EMA — bullish trend structure',
    ...
    triggers.push({
      ...
      strength: 'strong',
      description: 'Price < 9 EMA < 21 EMA — bearish trend structure',
      value: indicators.ema9
    })
  }

  // ── GOLDEN / DEATH CROSS ─────────────────────────────────────
  if (indicators.sma50 > indicators.sma200 && quote.price > indicators.sma50) {
    score += 10
    triggers.push({
      type: 'GOLDEN_CROSS_CONTEXT',
      strength: 'moderate',
      description: '50 SMA above 200 SMA — bullish long-term trend',
    ...
    isBullish = false
    triggers.push({
      type: 'DEATH_CROSS_CONTEXT',
      strength: 'moderate',
      description: '50 SMA below 200 SMA — bearish long-term trend',
      value: indicators.sma50
    })
  }
  ...
  if (indicators.nearSupport && indicators.bouncing) {
    ...
    triggers.push({
      type: 'SUPPORT_BOUNCE',
      strength: 'strong',
      description: `Bouncing off support at $${indicators.support?.toFixed(2)}`,
      value: indicators.support
    })
  }
  ...
  if (newsBoost > 0) {
    score += newsBoost
    triggers.push({
      type: 'POSITIVE_NEWS_CATALYST',
      strength: newsBoost >= 15 ? 'strong' : 'moderate',
      description: `Positive news sentiment boosting signal (+${newsBoost} pts)`,
      value: newsBoost
    })
  } else if (newsBoost < 0) {
    score += newsBoost // subtracts
    triggers.push({
      type: 'NEGATIVE_NEWS_RISK',
      strength: 'weak',
      description: `Negative news sentiment reducing signal (${newsBoost} pts)`,
      value: newsBoost
    })
  }
  ...
  if (optionsFlowScore > 80) {
    ...
    triggers.push({
      ...
      strength: 'strong',
      description: `Unusual options activity (score: ${optionsFlowScore})`,
      value: optionsFlowScore
    })
    ...
    triggers.push({
      ...
      strength: 'moderate',
      description: `Elevated options activity (score: ${optionsFlowScore})`,
      value: optionsFlowScore
    })
  }

  if (score < 30 || triggers.filter(t => t.type !== 'NEGATIVE_NEWS_RISK').length < 1) {
    return null
  }
