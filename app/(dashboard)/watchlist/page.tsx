...
const CSS = `
  ...
  .wl-input {
    width: 100%; background: rgba(232,224,212,0.04);
    border: 1px solid rgba(232,224,212,0.15);
    color: #e8e0d4; padding: 10px 14px;
  ...
  .wl-table th {
    ...
    text-transform: uppercase; color: rgba(232,224,212,0.4);
  ...
  .chip {
    ...
    border: 1px solid rgba(232,224,212,0.15); color: rgba(232,224,212,0.5);
  }
  ...
  .quick-ticker {
    ...
    color: rgba(232,224,212,0.5); cursor: pointer;
  ...
  .modal {
    background: #0f0f0f; border: 1px solid rgba(232,224,212,0.12);
...
export default function WatchlistPage() {
  ...
  const handleAddTicker = async () => {
    ...
    try {
      ...
      if (error) {
        if (error.code === '23505') alert('Already in watchlist')
      } else {
        setNewTicker('')
    ...
    <div style={{ padding: '32px 40px', fontFamily: '"DM Sans", sans-serif', color: '#e8e0d4', minHeight: '100vh', background: '#0a0a0a' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '32px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button className="wl-btn" onClick={() => refreshQuotes()}>
          ...
          <button className="wl-btn primary" onClick={() => setIsDialogOpen(true)}>
      ...
      <div style={{ border: '1px solid rgba(232,224,212,0.08)', marginBottom: '24px' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid rgba(232,224,212,0.07)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '11px', color: 'rgba(232,224,212,0.6)', letterSpacing: '0.06em' }}>
              ...
              {signals.size > 0 && ` · ${[...signals.values()].filter(s => s.signalType).length} signals found`}
            </span>
          </div>
          {signals.size === 0 && tickers.length > 0 && (
            <span style={{ fontFamily: '"DM Mono", monospace', fontSize: '10px', color: 'rgba(232,224,212,0.3)', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <Zap size={10} /> Click &quot;Scan Watchlist&quot; to see signals
            </span>
          )}
        </div>
        ...
        {watchlistLoading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '64px', textAlign: 'center' }}>
            ...
            <button className="wl-btn primary" onClick={() => setIsDialogOpen(true)}>
              <Plus size={11} /> Add Your First Stock
            </button>
          </div>
        ) : (
                      ...
                      <td style={{ textAlign: 'right' }}>
                        {rsi ? (
                          <div>
                            ...
                            <div style={{ fontFamily: '"DM Mono", monospace', fontSize: '9px', color: rsi < 30 ? '#7ec8a0' : rsi > 70 ? '#c87e7e' : 'rgba(232,224,212,0.35)', textAlign: 'right', letterSpacing: '0.04em' }}>
                        ...
                        {isScanning ? <Spinner className="h-3 w-3 mx-auto" /> :
                          signal?.signalType ? (
                            <span className={`chip ${signal.signalType === 'bullish_entry' ? 'bullish' : 'bearish'}`}>
                              ...
                              {signal.signalType === 'bullish_entry' ? 'BULLISH' : 'BEARISH'}
                        ...
                        {isScanning ? <Spinner className="h-3 w-3 ml-auto" /> :
                          signal?.targetPrice && signal?.stopLoss && quote ? (
                            <div style={{ fontFamily: '"DM Mono", monospace', fontSize: '11px' }}>
                              <div style={{ color: '#7ec8a0' }}>T ${signal.targetPrice.toFixed(2)} <span style={{ opacity: 0.6 }}>({((signal.targetPrice - quote.price) / quote.price * 100).toFixed(1)}%)</span></div>
                              <div style={{ color: '#c87e7e' }}>S ${signal.stopLoss.toFixed(2)} <span style={{ opacity: 0.6 }}>({((signal.stopLoss - quote.price) / quote.price * 100).toFixed(1)}%)</span></div>
